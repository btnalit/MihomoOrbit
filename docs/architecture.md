...
- **绑定是"先到先得"**：携带某个 token 的第一次合法心跳会原子性地把它带来的 `agentId` 与该后端绑定；此后同一后端收到携带不同 `agentId` 的心跳，一律返回 `409 AGENT_BINDING_FIXED`——即便 token 相同（详见 `docs/agent/troubleshooting.md`）
