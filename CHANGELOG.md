...
- **实时管理(M1)**:代理组页(选点/组测速/延迟徽标)、实时连接表(搜索、排序、kill)、实时日志(级别筛选)、运行时设置(模式/日志级别/局域网开关,仅作用于运行中的内核);由 collector 的 WebSocket 中继统一供数(引用计数、断路器、500 条日志环形缓冲)。
- **统一后端模型(M1c)**:一个后端 = API 通道(监控/管理)+ 可选 Agent 通道(配置编辑);能力按 `monitoring / management / configEdit` 分级门控页面;直连凭据与 agent 令牌互斥,杜绝双写。
- **配置编辑(M2)**:元数据驱动的 `config.yaml` 表单编辑;Go agent v2 经心跳捎带指令实现六步原子写回(基线哈希冲突检测、三份轮转备份、原子替换、reload、三重健康门、失败自动回滚);版本历史与一键回滚;敏感字段掩码、注释与锚点保真(CST 级替换)。
- **Providers 页(M1.5)**:规则/代理提供者列表与手动刷新;组页隐藏组过滤、非 Selector 组锁定、图标渲染。
- **管理页 UX(M1.7)**:连接/日志分页(日志改"最新在前");代理组自适应节点网格 + 组内搜索 + 折叠态延迟色条;配置编辑器前置代理联想输入(可选已有代理/组或自填)、ss 加密方式补齐官方 34 项全集、vmess 补 `zero`、规则策略下拉纳入内建策略与单节点、组成员可嵌套其他组。
- 发布自动化:`v*` tag 推送后 CI 在镜像推送成功后自动创建 GitHub Release(正文取自本文件对应段落,附 `docker-compose.yml` 与 `.env.example`),并校验 tag 与根 `package.json` 版本一致。

### 修复

- Agent 健康阈值(30s/8s)低于心跳间隔导致的状态抖动 → 75s,支持 `AGENT_HEARTBEAT_TIMEOUT_MS` / `AGENT_MANUAL_TEST_TIMEOUT_MS` 覆盖。
- 侧栏在弹层打开时跳动:基座滚动锁护甲写错目标元素(`html` vs `body`),body 被 `overflow:hidden` 后 sticky 锚点漂移 —— 修正覆盖规则。
- 连接页在零连接后端上永远停留在加载骨架(mihomo 零连接时 `connections: null` 帧被整帧丢弃)。
- 连接页 1s 重排导致行内 kill 按钮点击静默落空(指针按住期间冻结帧应用)。
- 添加/编辑后端对话框无高度上限,矮视口下保存/取消不可达。
- 前置代理联想下拉被自身打开手势误关("一闪而过")。
- 手机端陈旧 PWA 缓存导致旧界面常驻(Service Worker 缓存版本从未递增)。
- 桌面侧栏导航项溢出后无法滚动;新增页面样式与基座对齐(间距/阴影/暗色变体/空态)。
- i18n:改令牌报错显示原始 key、后端验证动画硬编码英文。

### 升级说明

- 首次启动会自动执行后端表 schema 迁移(M1c),日志中一次性迁移输出属预期。
- 配置编辑需为对应后端绑定 agent(`agent-v2.0.0`+);纯监控/管理后端无需 agent,行为不变。
- Agent 协议最低接受版本仍为 1,旧 agent 可继续心跳,但配置编辑要求 v2。
