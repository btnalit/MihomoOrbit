...
ask_start() {
	...
	if [[ $start_now =~ ^[Yy]$ ]]; then
		...
		echo ""
		echo "  Authentication is mandatory. First run has no access token yet —"
		echo "  find the one-time setup token in the collector logs:"
		echo "    docker logs -f mihomo-orbit"
		echo ""
